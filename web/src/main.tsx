...
function App(){
 ...
 return <div className="page">
  ...
  <main>
   ...
   {science&&<section className="notes"><strong>A wiring diagram is only the beginning.</strong>
    <p>Fox vs Fly is a playable experiment in connecting the MaleCNS connectome to a small platform fighter. The neural mode uses measured neuron positions and weighted connections from the <a href="https://male-cns.janelia.org/" target="_blank" rel="noreferrer">MaleCNS project</a>. Neurons without recorded positions still remain in the network.</p>
    <p>{dummy?'This demo route uses a deterministic rule-based controller. Its displayed activity is synthetic and does not come from the connectome.':'Game observations are converted into selected visual-like inputs. Activity propagates through the measured network, then authored descending and motor readouts map it to the controls shown below.'} The activation dynamics, sensory and motor interfaces, positive normalized coupling, and calibration are modeling choices made to produce a stable, playable system.</p>
    <p>The match is also an approximation: movement, attacks, damage, knockback, stocks, and respawning are implemented for this prototype, and the fighters use original artwork. This is a way to explore connectome-driven control, not a reconstruction of a fly’s body or a validated simulation of fly behavior.</p>
   </section>}
