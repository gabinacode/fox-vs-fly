...
export class BrainRenderer {
 ...
 setPopulationView(enabled:boolean){this.showPopulations=enabled;if(enabled){this.playing=false;this.time=0;this.yaw=this.targetYaw=.12;this.pitch=this.targetPitch=0;this.drive=this.targetDrive=0;this.burst=0;this.canvas.style.transform='';}}
 ...
 draw(dt:number){
  ...
  const lean=this.yaw-.12,breathe=Math.sin(this.time*1.55)*.003;if(this.showPopulations)this.canvas.style.transform='';else if(!this.captureCamera)this.canvas.style.transform=`translate3d(${(lean*18).toFixed(2)}px,${(this.pitch*36+Math.sin(this.time*.8)*1.2).toFixed(2)}px,0) rotate(${(lean*4).toFixed(2)}deg) scale(${(1+breathe+this.drive*.004+this.burst*.008).toFixed(4)})`;
