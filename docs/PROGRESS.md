...
## CURRENT MILESTONE
**Gameplay-coupled brain presentation makes the live soma cloud feel responsive.** The display now turns toward the Fox–Fly relationship, tilts with Fly vertical motion, blooms with current motor drive, and pulses on attack/hitlag edges, with subtle motion only while play is running. All motion is renderer-only; pause freezes it and the optional controller-population inspection remains fixed. WebGL uses an additive activity glow and the Canvas fallback shares pose/size response. One focused mapping test, production build, and Chrome gameplay smoke test pass; screenshot reviewed. See ACTIVITY_DISPLAY.md.

**LinkedIn capture mode for post-brain-reveal gameplay.** Deterministic Fox timelines drive the live MaleCNS V2 controller in a chrome-free full-bleed recorder (`?capture=hero-sequence` / `view=brain`). A headless search found a reproducible feint-then-commit chain (Fox move → activity/sensory change → motor Left+Attack → Fly attack at ticks 18/19/32/36). Music sync to Aphex Twin 180db_[130] is an editing property only; footage has no audio or baked text. Evidence: `data/capture-sequences.json`. Docs: CAPTURE.md.
...
- Fixed phone auto-pause: removed spurious `window.blur` pause (visibility hide only), stopped accruing schedule debt while waiting on the neural worker, and made HIT a non-focusing control so iOS does not scroll/blur mid-match.
...
## FAILING
None. Capture work verified: TypeScript 73/73 (5 capture tests), production Chrome 24/24. Native 5/5 and policy/Python checks also passed in `./scripts/verify.sh`; a stale vite preview on :42873 was cleared before the Chrome rerun.
...
2. Profile neural scaling before choosing shared graph batches, threads or GPU work.
3. Broaden controller evaluation to additional policies, spawn orientations and held-out phases before making robustness claims.
