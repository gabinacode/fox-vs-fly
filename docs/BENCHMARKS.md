...
A follow-up Chrome presentation fix removed per-frame CSS `transform` on the 139,662-soma WebGL canvas (Firefox largely ignored that restyle cost; Chrome’s ANGLE compositor did not). Live pose now updates camera uniforms shared with capture; uniform locations are cached and the context requests `powerPreference: high-performance`. After that change the isolated combat gate measured **52 game / 60 render / 26 neural fps**. Resolution and simulation cadence are unchanged.

## Blink brain paint cadence — 2026-09-23
The old interactive Blink profile capped the brain canvas at 10 paints/sec because a full 139,662-point draw could delay the neural worker. Desktop Blink now targets 30 paints/sec from a stable one-sixth hash sample; activity mapping only visits selected display points. At target cadence this uses about half the old points-per-second work, while 2× canvas resolution and the full measured geometry/model remain intact. Sampled draws can run during neural waits. Capture and population inspection still draw the full cloud. The footer now distinguishes brain paints from neural model updates.

In the final production-Chrome suite on this host, stationary combat measured **53 game / 37 arena-render / 22 brain-paint / 27 neural fps**; the earlier suite gate did not measure brain paints separately. The 22 fps paint count is more than twice the old 10 Hz policy but below the 30 Hz desktop target, as the scheduler continues to protect game/controller work. The browser suite passed 29/29. These measurements are host/run-specific, not a sustained-device or Firefox comparison.
